// minifilterScannerUser.cpp : 定义控制台应用程序的入口点。
//
...
#include <assert.h>
#include <fltuser.h>
#include "minifilterScannerUser.h"
#include <dontuse.h>
...
--*/
{
	...
	WCHAR request[256] = {L"request"};
	WCHAR receive[256] = { L"receive" };
	LPOVERLAPPED pOvlp;
	BOOL result;
	DWORD outSize;
	HRESULT hr;
...
#pragma warning(pop)
		...
		FilterSendMessage(Context->Port, request, sizeof(request), receive, sizeof(receive), &dwRtn);
		...
		printf("FilterSendMessage %s", receive);
	}
