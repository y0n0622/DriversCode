...
sfSetInformation(PDEVICE_OBJECT lpDevice, PIRP lpIrp)
{
    ...
    else
    {
		...
		WCHAR *pnewRename = NULL;
        if (lpIrpStack->Parameters.SetFile.FileInformationClass == FileRenameInformation)
        {
			//重命名的目标路径？
			//lpFileObject->FileName
			//RtlCopyMemory(&ustrDeviceName, &lpNameControl->Name, MAX_PATH*sizeof(WCHAR))
			pnewRename = (WCHAR*)lpIrp->AssociatedIrp.SystemBuffer; //+ sizeof(WCHAR);
        }

		R3UserResult = hipsGetResultFromUser(szOper, lpNameControl->Name.Buffer, pnewRename, User_DefaultNon);
