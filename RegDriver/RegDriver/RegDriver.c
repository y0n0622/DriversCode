...
VOID DriverUnload(PDRIVER_OBJECT pDriverObject)
{
	...
	NTSTATUS ntStatus = ntDeleteValueKey(L"\\Registry\\Machine\\SoftWare\\y0n");
	if (!NT_SUCCESS(ntStatus))
	{
		DbgPrint("ntSetValueKey() failed:%x\n", ntStatus);
		return;
	}
	ntDeleteKey(L"\\Registry\\Machine\\SoftWare\\y0n");
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pRegPath)
{
	...
	RegOper(L"\\Registry\\Machine\\SoftWare\\y0n");
...
NTSTATUS ntCreateKey(WCHAR *szKey)
{
	...
	InitializeObjectAttributes(&objectAttributes, &uRegKey, OBJ_CASE_INSENSITIVE, NULL, NULL);
	...
	InitializeObjectAttributes(&subObjectAttributes, &subRegKey, OBJ_CASE_INSENSITIVE, hRegister, NULL);
...
NTSTATUS ntEnumerateSubKey(WCHAR *szKey)
{
	...
	ntStatus = ZwQueryKey(hRegister, KeyFullInformation, NULL, 0, &ulSize);
	if (STATUS_BUFFER_OVERFLOW != ntStatus && STATUS_BUFFER_TOO_SMALL != ntStatus)
	{
		return ntStatus;
	}

	pfi =(PKEY_FULL_INFORMATION)ExAllocatePoolWithTag(PagedPool, ulSize, 'SGER');
	if (pfi == NULL)
	{
		ZwClose(hRegister);
	...
	ntStatus = ZwQueryKey(hRegister, KeyFullInformation, pfi, ulSize, &ulSize);
	if (!NT_SUCCESS(ntStatus))
	{
		ExFreePool(pfi);
		ZwClose(hRegister);
	...
	for (i = 0; i < pfi->SubKeys; i++)
	{
		...
		ntStatus = ZwEnumerateKey(hRegister, i, KeyBasicInformation, NULL, 0, &ulSize);
		if (STATUS_BUFFER_OVERFLOW != ntStatus && STATUS_BUFFER_TOO_SMALL != ntStatus)
		{
			ZwClose(hRegister);
			ExFreePool(pfi);
		...
		pbi = (PKEY_BASIC_INFORMATION)ExAllocatePoolWithTag(PagedPool, ulSize, 'SGER');
		if (pbi == NULL)
		{
			ZwClose(hRegister);
			return STATUS_INSUFFICIENT_RESOURCES;
		...
		ntStatus = ZwEnumerateKey(hRegister, i, KeyBasicInformation, pbi, ulSize, &ulSize);
		if (!NT_SUCCESS(ntStatus))
		{
			ZwClose(hRegister);
			ExFreePool(pfi);
...
NTSTATUS ntSetValueKey(WCHAR *szKey)
{
	...
	RtlInitUnicodeString(&ValueName, L"y0n1");
	ulValue = 1000;
	ZwSetValueKey(hRegister, &ValueName, 0, REG_DWORD, &ulValue, sizeof(ulValue));

	RtlInitUnicodeString(&ValueName, L"y0n2");
	ZwSetValueKey(hRegister, &ValueName, 0, REG_SZ, strValue, (ULONG)wcslen(strValue)*sizeof(WCHAR) + sizeof(WCHAR));

	RtlInitUnicodeString(&ValueName, L"y0n3");
...
NTSTATUS ntQueryValueKey(WCHAR *szKey)
{
	...
	{
		UNICODE_STRING uStr;
		uStr.Length = (USHORT)pvpi->DataLength;
		uStr.MaximumLength = (USHORT)pvpi->DataLength;
		uStr.Buffer = (PWCH)pvpi->Data;
...
NTSTATUS ntEnumerateSubValueKey(WCHAR *szKey)
{
	...
	{
		ntStatus = ZwEnumerateValueKey(hRegister, i, KeyValueBasicInformation, NULL, 0, &ulSize);
		if (STATUS_BUFFER_OVERFLOW != ntStatus && STATUS_BUFFER_TOO_SMALL != ntStatus)
		{
			ZwClose(hRegister);
			ExFreePool(pfi);
		...
		pvbi = (PKEY_VALUE_BASIC_INFORMATION)ExAllocatePoolWithTag(PagedPool, ulSize, 'SGER');
		if (pvbi == NULL)
		{
			ZwClose(hRegister);
			ExFreePool(pfi);
		...
		ntStatus = ZwEnumerateValueKey(hRegister, i, KeyValueBasicInformation, pvbi, ulSize, &ulSize);
		if (!NT_SUCCESS(ntStatus))
		{
			ZwClose(hRegister);
			ExFreePool(pfi);
